...
static_assert(kForwardBlockSize <= kMaxStaticLoopBlock,
              "Tests should not request block sizes above the cap");
constexpr std::size_t kRemainderBlockSize = 3;
static_assert(kRemainderBlockSize <= kMaxStaticLoopBlock,
              "Tests should not request block sizes above the cap");

static_assert(compute_range_count<0, 0, 1>() == 0,
              "Zero-length ranges should not iterate");
static_assert(compute_range_count<0, 4, 1>() == 4,
              "Positive ranges compute the expected count");
static_assert(compute_range_count<2, 11, 3>() == 3,
              "Positive ranges honour custom steps");
static_assert(compute_range_count<5, 5, -1>() == 0,
              "Zero-length negative ranges yield zero iterations");
static_assert(compute_range_count<5, 1, -1>() == 4,
              "Negative ranges compute the expected count");
static_assert(compute_range_count<8, -1, -3>() == 3,
              "Negative ranges honour custom steps");
...
static_assert(
    compute_default_static_loop_block_size<0, kMaxStaticLoopBlock, 1>() ==
        kMaxStaticLoopBlock,
    "Ranges matching the cap use the maximum default block size");
static_assert(
    compute_default_static_loop_block_size<0, kMaxStaticLoopBlock + 10, 1>() ==
        kMaxStaticLoopBlock,
    "Large ranges clamp the default block size to the inclusive maximum");
static_assert(
    compute_default_static_loop_block_size<0, 4096, 1>() ==
        kMaxStaticLoopBlock,
    "Very large ranges clamp the default block size to kMaxStaticLoopBlock");
static_assert(compute_default_static_loop_block_size<
                  kMaxStaticLoopBlock + 5, 0, -1>() == kMaxStaticLoopBlock,
...
constexpr auto compute_squares_default_unroll() {
  std::array<int, 4> values{};
  poet::static_for<0, 4, 1>(
      square_functor<std::array<int, 4>>{&values});
  return values;
}
...
constexpr auto compute_squares_custom_unroll() {
  std::array<int, 5> values{};
  poet::static_for<0, 5, 1, kForwardBlockSize>(
      square_functor<std::array<int, 5>>{&values});
  return values;
}

constexpr auto compute_squares_clamped_default() {
  std::array<int, kMaxStaticLoopBlock + 10> values{};
  poet::static_for<0, kMaxStaticLoopBlock + 10>(
      square_functor<std::array<int, kMaxStaticLoopBlock + 10>>{&values});
  return values;
}
...
static_assert(clamped_default_squares.back() ==
              static_cast<int>((kMaxStaticLoopBlock + 9) *
                               (kMaxStaticLoopBlock + 9)));
...
} // namespace (merged constexpr tests)
...
TEST_CASE("static_for handles large iteration counts", "[static_for][limits]") {
  ...
  poet::static_for<0, kSpan, 1>(
      square_functor<std::array<int, kSpan>>{&values});
