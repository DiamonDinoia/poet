...
struct Case {
    std::string name;
    int expect;// FMAs per back-edge, or -1 for report-only
...
auto is_fma(const std::string &mnemonic, const std::string &operands) -> bool {
    ...
    return std::regex_match(mnemonic, call_op) && std::regex_search(operands, fma_callee);
}
...
auto find_loops(const std::vector<std::string> &lines, const std::string &name, bool &found) -> std::vector<int> {
    const std::regex start("^_?" + name + ":");
    std::size_t begin = lines.size();
    for (std::size_t i = 0; i < lines.size(); ++i) {
        if (std::regex_search(first_token(lines[i]), start)) {
            begin = i;
            break;
        }
    }
    found = begin < lines.size();
    if (!found) { return {}; }
    ...
    const auto targets_of = [&](std::size_t i) {
        ...
        for (auto it = std::sregex_iterator(text.begin(), text.end(), token_re); it != std::sregex_iterator(); ++it) {
            ...
            if (label != labels.end()) { out.push_back(next[label->second]); }
        }
        return out;
    };
    ...
    for (std::size_t i = 0; i < span; ++i) {
        ...
        for (auto it = std::sregex_iterator(text.begin(), text.end(), token_re); it != std::sregex_iterator(); ++it) {
            const auto label = labels.find(it->str());
            if (label == labels.end() || label->second > i) { continue; }
...
auto main(int argc, char **argv) -> int {
    ...
    const std::vector<Case> cases = { { "sf1_runtime", 1, 1 },
        { "sf2_runtime", 2, 1 },
        { "sf4_runtime", 4, 1 },
        { "sf8_runtime", 8, 1 },
        { "sf1_const", 1, 1 },
    ...
    for (const std::string &standard : standards) {
        for (const std::string &flags : flag_sets) {
            ...
            for (const Case &item : cases) {
                bool found = false;
                const std::vector<int> loops = find_loops(lines, item.name, found);
                if (!found) {
                ...
                int widest = 0;
                for (const int fmas : loops) { widest = std::max(widest, fmas); }
                const bool report_only = item.expect < 0;
                const bool ok =
                  report_only || (static_cast<int>(loops.size()) == item.backedges && widest == item.expect);
                std::cout << "exact-unroll " << (report_only ? "NOTE " : (ok ? "PASS " : "FAIL ")) << " [" << cell
                          << "] " << item.name << " back-edges=" << loops.size() << " fma/back-edge=" << widest;
                if (report_only) {
                    std::cout << " (no barrier: the flag set unrolls a one-FMA loop by " << widest << ")\n";
                } else {
                    std::cout << " expect back-edges=" << item.backedges << " fma/back-edge=" << item.expect << "\n";
                    ++checked;
                    if (!ok) { ++failures; }
                }
            }
