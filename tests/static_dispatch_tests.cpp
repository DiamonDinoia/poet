...
struct mover {
  template <int X>
  std::unique_ptr<int> operator()(int base) const { return std::make_unique<int>(base + X); }
};

struct receiver {
  template <int X>
  int operator()(std::unique_ptr<int> p) const { return X + *p; }
};

struct probe {
  int *out;
  template <int X, int Y>
  void operator()(int base) const { *out = base + X + Y; }
};
...
TEST_CASE("dispatch handles boundary values in ranges", "[static_dispatch]") {
  ...
  SECTION("minimum boundary") {
    auto params = std::make_tuple(DispatchParam<make_range<0, 3>>{0},
                                  DispatchParam<make_range<-2, 1>>{-2});
    dispatch(vector_dispatcher{&values}, params, 5);
    REQUIRE(values == std::vector<int>{3});
  }

  SECTION("maximum boundary") {
    auto params = std::make_tuple(DispatchParam<make_range<0, 3>>{3},
                                  DispatchParam<make_range<-2, 1>>{1});
    dispatch(vector_dispatcher{&values}, params, 5);
...
TEST_CASE("dispatch handles ranges crossing zero", "[static_dispatch]") {
  std::vector<int> values;
  auto params = std::make_tuple(DispatchParam<make_range<-3, 3>>{0},
                                DispatchParam<make_range<-1, 1>>{0});
...
TEST_CASE("dispatch handles void return type explicitly", "[static_dispatch]") {
  std::vector<int> values;
  auto params = std::make_tuple(DispatchParam<make_range<1, 2>>{1},
...
TEST_CASE("dispatch handles multiple out-of-range parameters",
          "[static_dispatch]") {
  std::vector<int> values;
  auto params = std::make_tuple(DispatchParam<make_range<0, 2>>{10},
                                DispatchParam<make_range<5, 7>>{15});
...
TEST_CASE("dispatch handles non-contiguous sequences",
          "[static_dispatch][non-contiguous]") {
  ...
  SECTION("matches first element") {
    auto params = std::make_tuple(
        DispatchParam<NonContiguousSeq>{1},
        DispatchParam<std::integer_sequence<int, 2, 5>>{2});
  ...
  SECTION("matches middle element") {
    auto params = std::make_tuple(
        DispatchParam<NonContiguousSeq>{7},
        DispatchParam<std::integer_sequence<int, 2, 5>>{5});
  ...
  SECTION("matches last element") {
    auto params = std::make_tuple(
        DispatchParam<NonContiguousSeq>{12},
        DispatchParam<std::integer_sequence<int, 2, 5>>{2});
...
TEST_CASE("dispatch handles mixed contiguous and non-contiguous sequences",
          "[static_dispatch][non-contiguous]") {
  using NonContiguousSeq = std::integer_sequence<int, 0, 10, 20>;

  std::vector<int> values;
  auto params = std::make_tuple(
      DispatchParam<NonContiguousSeq>{10},
      DispatchParam<make_range<1, 3>>{2});
...
TEST_CASE("dispatch deterministic with duplicate sequence values",
          "[static_dispatch][duplicates]") {
  int out = 0;
...
TEST_CASE("dispatch handles negative non-contiguous sequences",
          "[static_dispatch][non-contiguous]") {
...
TEST_CASE("dispatch preserves return value types correctly",
          "[static_dispatch]") {
