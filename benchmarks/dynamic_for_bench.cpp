...
int dynamic_for_sum(std::size_t begin, std::size_t end) {
  int total = 0;
  poet::dynamic_for<Unroll>(begin, end, [&](std::size_t index) {
    total += static_cast<int>(index);
  });
  return total;
}

void benchmark_range(ankerl::nanobench::Bench &bench, std::size_t begin,
                     std::size_t end, const char *label_suffix) {
  const std::string unroll8_label =
      std::string("dynamic_for ") + label_suffix + " (unroll 8)";
  bench.run(unroll8_label, [=] {
    const auto result = dynamic_for_sum(begin, end);
    ankerl::nanobench::doNotOptimizeAway(result);
  });

  const std::string unroll16_label =
      std::string("dynamic_for ") + label_suffix + " (unroll 16)";
  bench.run(unroll16_label, [=] {
    const auto result = dynamic_for_sum<16>(begin, end);
    ankerl::nanobench::doNotOptimizeAway(result);
  });

  const std::string manual_label =
      std::string("manual for ") + label_suffix;
