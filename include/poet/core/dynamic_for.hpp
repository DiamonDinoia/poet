...
/// 1. A runtime loop over full blocks of `Unroll` iterations.
/// 2. A compile-time-unrolled block body (`execute_runtime_block`).
/// 3. Tail handling through `dispatch` for 1..Unroll-1 remaining iterations.
/// 4. A tiny-range fast path (`count < Unroll`) that skips dispatch overhead.
...
namespace detail {
    ...
    /// \brief Adapter invoked by `static_for` when emitting a runtime block.
    ...
    /// \brief Computes iteration count for strides other than +1.
    template<typename T>
    ...
    /// \brief Emits one compile-time-unrolled block of `BlockSize` calls.
    template<typename Func, typename T, std::size_t BlockSize>
